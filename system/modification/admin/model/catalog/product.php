...
class ModelCatalogProduct extends Model {
	public function addProduct($data) {
		...
		if (isset($data['product_additional'])) {
			foreach ($data['product_additional'] as $additional_id) {
				$this->db->query("DELETE FROM " . DB_PREFIX . "product_additional WHERE product_id = '" . (int)$product_id . "' AND additional_id = '" . (int)$additional_id . "'");
				$this->db->query("INSERT INTO " . DB_PREFIX . "product_additional SET product_id = '" . (int)$product_id . "', additional_id = '" . (int)$additional_id . "'");
				$this->db->query("DELETE FROM " . DB_PREFIX . "product_additional WHERE product_id = '" . (int)$additional_id . "' AND additional_id = '" . (int)$product_id . "'");
				$this->db->query("INSERT INTO " . DB_PREFIX . "product_additional SET product_id = '" . (int)$additional_id . "', additional_id = '" . (int)$product_id . "'");
			}
		}
	...
	public function editProduct($product_id, $data) {
		...
		$this->db->query("DELETE FROM " . DB_PREFIX . "product_additional WHERE product_id = '" . (int)$product_id . "'");
		$this->db->query("DELETE FROM " . DB_PREFIX . "product_additional WHERE additional_id = '" . (int)$product_id . "'");
	...
	public function copyProduct($product_id) {
		...
		if ($query->num_rows) {
			...
			$data = array_merge($data, array('product_additional' => $this->getProductAdditional($product_id)));
			...
			$next_code = $max_mod + 1;
			$data["model"] = $data["manufacturer_id"].$next_code ;
/*			$p = strpos($data["model"], "-");
	...
	public function getProductAdditional($product_id) {
		...
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_additional WHERE product_id = '" . (int)$product_id . "'");

		foreach ($query->rows as $result) {
			$product_additional_data[] = $result['additional_id'];
		}
