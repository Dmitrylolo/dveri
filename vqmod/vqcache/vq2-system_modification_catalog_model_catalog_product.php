...
class ModelCatalogProduct extends Model {
	...
	public function getProductAdditional ($product_id) {
		...
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_additional pr LEFT JOIN " . DB_PREFIX . "product p ON (pr.additional_id = p.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE pr.product_id = '" . (int)$product_id . "' AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'");

		foreach ($query->rows as $result) {
			$product_data[$result['additional_id']] = $this->getProduct($result['additional_id']);
		}
